...
해양진흥공사의 위기 관리 고도화 프로젝트를 진행하는 동안, 해운 지수(index)의 데이터가 부족하여 어려움을 겪은 적이 있습니다. 이를 해결하기 위해 Backcasting LSTM을 사용하여 부족한 과거 데이터의 index 값을 10분기 이상 확보하였습니다.

또한, 회사에서 현재 진행 중인 '위기인식지수 AI 고도화 프로젝트'에서는 데이터 불균형 문제가 심각했습니다. 딥러닝 모델을 적용해 학습시키면 정확도는 높았으나 recall score가 매우 낮아지는 문제가 발생했습니다. 이를 해결하기 위해 SMOTE와 ADASYN 기법을 사용하여 주요 데이터의 0.5배만큼 데이터를 증강하였고, 이를 통해 딥러닝 모델의 성능을 향상시키는 데 성공했습니다. 이로써 정확도뿐만 아니라 recall score도 함께 높아졌습니다.
...
이후, AI 관련 저서를 검수하는 동안 여러 매체를 통해 머신러닝과 딥러닝에 대해 심도 있게 공부했습니다:
- 위키독스의 AI 관련 저서
- 유튜브 강의
- 개인적으로 딥러닝, 머신러닝, 통계 책 구매 및 학습

이를 통해 다음과 같은 AI 관련 기술 스택을 보유하고 있습니다:
- **프레임워크**: TensorFlow, PyTorch, scikit-learn
- **사용 언어**: Python
- **데이터 전처리 및 시각화 라이브러리**: Matplotlib, Plotly, Seaborn, Pandas, NumPy

다음의 머신러닝 및 딥러닝 지식을 습득하였습니다:
...
빠르게 업무에 적응하는 것이 우선입니다. 보고서 작성 과정에 참여하고, 필요한 데이터와 정보를 분석하여 전반적인 업무 구조와 프로젝트 흐름을 이해하겠습니다. 이를 통해 팀원들과 적극적으로 소통하며 업무 흐름을 신속하게 파악하여 맡은 직무에 빠르게 적응하겠습니다.
